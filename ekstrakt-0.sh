...
extract_semrush(){
    ...
    while IFS= read -r semrushdata; do
        ...
        if [[ $line -eq 50004 && $line != 30004 && $line != 10004 ]]; then
            ...
            done < "$semrushdata" | grep -v "Keyword" | sed 's/"//g' | sed "s/[]\[\/\|\():;'_*^\&\%\$\#\@\!\+\`\~\"{}\<1234567890>?=-]//g" > "$out"
...
semrush_business(){
    ...
    done < "$file" | grep -v "Keyword" | sed 's/"//g' | sed "s/[]\[\/\|\():;'_*^\&\%\$\#\@\!\+\`\~\"{}\<1234567890>?=-]//g" > "$output"
}
...
google_plan_all(){   
    ...
    while IFS= read -r keyworddata; do
        ...
        done < "$keyworddata" | grep -v "Keyword" | sed 's/"//g' | sed "s/[]\[\/\|\():;'_*^\&\%\$\#\@\!\+\`\~\"{}\<1234567890>?=-]//g" > "$out"
